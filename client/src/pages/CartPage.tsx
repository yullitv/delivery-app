...
const CartPage = () => {
    ...
    switch (name) {
      case "email":
        if (!validateEmail(value))
          error = "Invalid email format (e.g. user@mail.com)";
        break;
      case "phone":
      ...
      case "address":
        if (value.trim().length < 10)
          error = "Address must be at least 10 chars";
        break;
    }
  ...
  if (items.length === 0) {
    return (
      <Card className="text-center py-20 animate-in zoom-in duration-300">
        <ShoppingBag size={64} className="mx-auto text-gray-200 mb-4" />
        <h2 className="text-2xl font-bold text-gray-800 mb-2">
          Your cart is empty
        </h2>
        <p className="text-gray-500 mb-8">
          Pick something delicious and come back!
        </p>
        <Button
          variant="outline"
          onClick={() => navigate("/")}
