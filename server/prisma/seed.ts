...
async function main() {
  ...
  const burgerNames = ["Angus Burger", "Bacon King", "Classic Cheeseburger", "Double Whopper", "Extreme Spicy", "Fish Fillet", "Guacamole Deluxe", "Hawaiian Burger", "Iberico Pork Burger"];
  const drinkNames = ["Apple Juice", "Blue Lagoon", "Coca-Cola", "Diet Sprite", "Espresso", "Fresh Lemonade", "Ginger Ale", "Hot Chocolate", "Iced Latte"];
  const dessertNames = ["Apple Pie", "Brownie", "Cheesecake", "Donut", "Eclair", "Fruit Salad", "Gelato", "Honey Cake"];
  ...
  for (const shopData of shops) {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      if (category === "Burgers") baseName = burgerNames[i % burgerNames.length] as string;
      else if (category === "Drinks") baseName = drinkNames[i % drinkNames.length] as string;
      else baseName = dessertNames[i % dessertNames.length] as string;
